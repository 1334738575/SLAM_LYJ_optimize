...
namespace OPTIMIZE_LYJ
{
    namespace OPTIMIZE_BASE
    {
        ...
        OPTIMIZE_LYJ_API Eigen::Vector2d Point2Image(double *_K, const Eigen::Matrix<double, 3, 4> &_Tcw, double *_Pw);
        ...
        OPTIMIZE_LYJ_API Eigen::Matrix3d skew_symmetric(const Eigen::Vector3d &v);
        OPTIMIZE_LYJ_API Eigen::Matrix3d NormalizeRotation(const Eigen::Matrix3d &R);
        ...
        //---- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- - ;
        OPTIMIZE_LYJ_API void cal_jac_errT_T(const m44 &priTcw, const m44 &Tcw, v6d &err, m66 &jac);
        //---- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- - ;
        ...
        OPTIMIZE_LYJ_API Eigen::Matrix<double, 2, 6> compute_se3_jacobian(
        ...
        // 生成3x3反对称矩阵
        OPTIMIZE_LYJ_API Eigen::Matrix3d hat(const Eigen::Vector3d &v);
        // 从反对称矩阵恢复向量
        OPTIMIZE_LYJ_API Eigen::Vector3d vee(const Eigen::Matrix3d &M);
        // SE3 对数映射（将变换矩阵转换为李代数）
        OPTIMIZE_LYJ_API se3 se3_log(const SE3 &T);
        // SE3 指数映射（将李代数转换为变换矩阵）
        OPTIMIZE_LYJ_API SE3 se3_exp(const se3 &xi);
        // 构造伴随矩阵 ad(ξ)
        OPTIMIZE_LYJ_API Eigen::Matrix<double, 6, 6> ad_se3(const se3 &xi);
        // 计算先验位姿误差的雅可比矩阵
        OPTIMIZE_LYJ_API Eigen::Matrix<double, 6, 6> compute_prior_jacobian(
            const SE3 &T_current, // 当前位姿 (4x4)
            const SE3 &T_prior,   // 先验位姿 (4x4)
            se3 &_err);

        // 反对称矩阵生成
        OPTIMIZE_LYJ_API Eigen::Matrix3d skew(const Eigen::Vector3d &v);
        // Plücker直线结构体
        struct PluckerLine
        {
        ...
        // 计算SE3的伴随矩阵
        OPTIMIZE_LYJ_API Eigen::Matrix<double, 6, 6> adjoint_SE3(const SE3 &T);
        // 三维线投影到图像直线
        OPTIMIZE_LYJ_API Eigen::Vector3d projectLineToImage(
            const PluckerLine &L_camera,
            const Eigen::Matrix3d &K_invT);
        OPTIMIZE_LYJ_API void computeJacobians(
            const SE3 &T,                 // 当前位姿
        ...
        OPTIMIZE_LYJ_API Eigen::Matrix3d EulerToRot(const double &r, const double &p, const double &w);
        OPTIMIZE_LYJ_API void computeJacRelPose3d_Pose3d_Pose3d(
